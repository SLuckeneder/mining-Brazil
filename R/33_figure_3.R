...
### yearly estimates
p_dat <- read.csv(file = paste0("data/cem/sdm_impact_estimates/summaries/", impacts_gdp_yearly))

p_dat <- p_dat %>% dplyr::filter(Variable %in% variables_industrial_yearly) %>%
  dplyr::mutate(freq = "yearly") %>%
  dplyr::mutate(unit = "Effect of industrial mining on GDP per capita growth") 
...
p_gdp_year <- p_dat %>%
  ...
  ggplot2::scale_y_continuous(limits = c(-2.6, 4.2), expand = c(0, 0)) +
...
p_dat <- read.csv(file = paste0("data/cem/sdm_impact_estimates/summaries/", impacts_gdp_pooled))
...
# Figure 3b garimpo mining impact estimates ----------------------------
...
p_dat <- p_dat %>% dplyr::filter(Variable %in% variables_garimpo_yearly) %>%
  dplyr::mutate(freq = "yearly") %>%
  dplyr::mutate(unit = "Effect of garimpo mining on GDP per capita growth") 
...
# 
# ggplot2::ggsave("figure_3a.png",
#                 plot = p_gdp_industrial, device = "png",
#                 path = paste0("./figures/presentations"),
#                 scale = 1, width = 300, height = 100, units = "mm")
# 
# ggplot2::ggsave("figure_3b.png",
#                 plot = p_gdp_garimpo, device = "png",
#                 path = paste0("./figures/presentations"),
#                 scale = 1, width = 300, height = 100, units = "mm")
# 
...
# # for thesis --------------------------------------------------------------
# 
# p_merge <- cowplot::plot_grid(p_gdp_industrial, p_gdp_garimpo, nrow = 2, rel_heights = c(1/2, 1/2), labels = c("A", "B"), label_size = 20) +
#   theme(plot.background = element_rect(fill = "white", colour = NA))
# 
# ggplot2::ggsave("figure_3.png",
#                 plot = p_merge, device = "png",
...
# # for defensio ------------------------------------------------------------
# 
# p_merge <- cowplot::plot_grid(p_gdp_industrial, p_gdp_garimpo, nrow = 2, rel_heights = c(1/2, 1/2), label_size = 20) +
#   theme(plot.background = element_rect(fill = "white", colour = NA))
# 
# ggplot2::ggsave("paper-2_results-1.pdf",
