...
matching_model <- "baseline" # k to k matching makes no sense for the spatial model, because it removes almost all neighbours
mining_type <- "garimpo" # "industrial", "garimpo"
dependent <- "abs" # "gdp", "rel", "abs"
...
if (mining_type == "industrial"){
  ...
  if(! file.exists(paste0("./data/W/W_k", knn_set, "_matching_", matching_model, "_industrial.RData"))){
    ...
    save(W_k, file = paste0("./data/W/W_k", knn_set, "_matching_", matching_model, "_industrial.RData"))
  } else {load(paste0("./data/W/W_k", knn_set, "_matching_", matching_model, "_industrial.RData"))}
  
} else {
  
  if(! file.exists(paste0("./data/W/W_k", knn_set, "_matching_", matching_model, "_garimpo.RData"))){
    ...
    save(W_k, file = paste0("./data/W/W_k", knn_set, "_matching_", matching_model, "_garimpo.RData"))
  } else {load(paste0("./data/W/W_k", knn_set, "_matching_", matching_model, "_garimpo.RData"))}
...
if (mining_type == "industrial"){
  ...
  # Economic growth model (dependent is 5y average annual GDP)
  ...
  X <- M_matched_industrial %>%
    dplyr::select(gdp_capita_log, pop_growth, pop_dens, educ,
                  ...
                  mining_industrial,
                  year) %>%
    dplyr::mutate(mining_industrial_x_pre_2010 = ifelse(year %in% c(2000:2009), mining_industrial, 0),
                  mining_industrial_x_since_2010 = ifelse(year > 2009, mining_industrial, 0)) %>%
    dplyr::select(-mining_industrial) %>%
    as.matrix()
  YX <- cbind(Y, X)
  write.csv(YX, file = paste0("./data/cem/cem_model_input/YX_gdp_pooled_industrial_hectares_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
  
  # Relative forest loss model (dependent is ha per km2 of forest in municipality)
  Y <- as.numeric(M_matched_industrial$natural_forest_loss_ha_km2)
  X <- M_matched_industrial %>% 
    ...
    as.matrix()
  YX <- cbind(Y, X)
  write.csv(YX, file = paste0("./data/cem/cem_model_input/YX_rel_pooled_industrial_hectares_", matching_model, "_",from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
  
  # Absolute forest loss model (dependent is annual deforestation absolute ha)
  Y <- as.numeric(M_matched_industrial$natural_forest_loss)
  X <- M_matched_industrial %>% 
    dplyr::select(gdp_capita_growth,
                  share_Agriculture, `share_Forest.Formation`, `share_Forest.Plantation`, share_Grassland, share_Pasture,
                  mav_Agriculture_Grassland_log, mav_Agriculture_Pasture_log, mav_Grassland_Agriculture_log, mav_Grassland_Pasture_log, mav_Pasture_Agriculture_log, mav_Pasture_Grassland_log,
    ...
    as.matrix()
  YX <- cbind(Y, X)
  write.csv(YX, file = paste0("./data/cem/cem_model_input/YX_abs_pooled_industrial_hectares_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
  ...
  X <- M_matched_garimpo %>%
    dplyr::select(gdp_capita_log, pop_growth, pop_dens, educ,
                  ...
                  mining_garimpo,
                  year) %>%
    dplyr::mutate(mining_garimpo_x_pre_2010 = ifelse(year %in% c(2000:2009), mining_garimpo, 0),
                  mining_garimpo_x_since_2010 = ifelse(year > 2009, mining_garimpo, 0)) %>%
    dplyr::select(-mining_garimpo) %>%    
    as.matrix()
  YX <- cbind(Y, X)
  write.csv(YX, file = paste0("./data/cem/cem_model_input/YX_gdp_pooled_garimpo_hectares_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
  
  # Relative forest loss model yearly (dependent is ha per km2 of forest in municipality)
  Y <- as.numeric(M_matched_garimpo$natural_forest_loss_ha_km2)
  X <- M_matched_garimpo %>% 
    dplyr::select(gdp_capita_growth,
                  share_Agriculture, `share_Forest.Formation`, `share_Forest.Plantation`, share_Grassland, share_Pasture,
                  mav_Agriculture_Grassland_log, mav_Agriculture_Pasture_log, mav_Grassland_Agriculture_log, mav_Grassland_Pasture_log, mav_Pasture_Agriculture_log, mav_Pasture_Grassland_log,
    ...
    as.matrix()
  YX <- cbind(Y, X)
  write.csv(YX, file = paste0("./data/cem/cem_model_input/YX_rel_pooled_garimpo_hectares_", matching_model, "_",from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
  
  # Absolute forest loss model yearly (dependent is annual deforestation absolute ha)
  Y <- as.numeric(M_matched_garimpo$natural_forest_loss)
  X <- M_matched_garimpo %>% 
    dplyr::select(gdp_capita_growth,
                  share_Agriculture, `share_Forest.Formation`, `share_Forest.Plantation`, share_Grassland, share_Pasture,
                  mav_Agriculture_Grassland_log, mav_Agriculture_Pasture_log, mav_Grassland_Agriculture_log, mav_Grassland_Pasture_log, mav_Pasture_Agriculture_log, mav_Pasture_Grassland_log,
    ...
    as.matrix()
  YX <- cbind(Y, X)
  write.csv(YX, file = paste0("./data/cem/cem_model_input/YX_abs_pooled_garimpo_hectares_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
if(mining_type == "industrial"){
  ...
  if(dependent == "gdp"){
    YX <- read.csv2(file = paste0("./data/cem/cem_model_input/YX_gdp_pooled_industrial_hectares_", matching_model, "_2005-2015_5y.csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
  if(dependent == "rel"){
    YX <- read.csv2(file = paste0("./data/cem/cem_model_input/YX_rel_pooled_industrial_hectares_", matching_model, "_2005-2015_5y.csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
  if(dependent == "abs"){
    YX <- read.csv2(file = paste0("./data/cem/cem_model_input/YX_abs_pooled_industrial_hectares_", matching_model, "_2005-2015_5y.csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
  
} else {
  
  if(dependent == "gdp"){
    YX <- read.csv2(file = paste0("./data/cem/cem_model_input/YX_gdp_pooled_garimpo_hectares_", matching_model, "_2005-2015_5y.csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
  if(dependent == "rel"){
    YX <- read.csv2(file = paste0("./data/cem/cem_model_input/YX_rel_pooled_garimpo_hectares_", matching_model, "_2005-2015_5y.csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
  if(dependent == "abs"){
    YX <- read.csv2(file = paste0("./data/cem/cem_model_input/YX_abs_pooled_garimpo_hectares_", matching_model, "_2005-2015_5y.csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
...
save(store, file = paste0("data/intermediary/MCMC_draws_matching/", dependent, "_pooled_hectares_", mining_type, "_", matching_model, "_", Sys.time(), ".Rdata"))
...
save(store_impacts, file = paste0("data/cem/sdm_impact_estimates/draws/impacts_", dependent, "_pooled_hectares_", mining_type, "_", matching_model, "_", Sys.time(), ".Rdata"))
...
# save as Excel
write.csv(impact_summary, file = paste0("data/cem/sdm_impact_estimates/summaries/", dependent, "_pooled_hectares_", mining_type, "_", matching_model, "_" , ntot, "draws_", Sys.time(), ".csv"), row.names = FALSE)
