...
for (i in seq_along(window_yrs_alternatives)){
  ...
  if (mining_type == "industrial"){
    ...
    X <- M_matched_industrial %>%
      ...
      as.matrix()
    YX <- cbind(Y, X)
    write.csv(YX, file = paste0("./data/cem/cem_model_input/SI/YX_gdp_yearly_industrial_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
    ...
    X <- M_matched_garimpo %>%
      ...
      as.matrix()
    YX <- cbind(Y, X)
    write.csv(YX, file = paste0("./data/cem/cem_model_input/SI/YX_gdp_yearly_garimpo_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
  ...
  # read data
  if(mining_type == "industrial"){
      YX <- read.csv2(file = paste0("./data/cem/cem_model_input/SI/YX_gdp_yearly_industrial_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  } else {
      YX <- read.csv2(file = paste0("./data/cem/cem_model_input/SI/YX_gdp_yearly_garimpo_", matching_model, "_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
  }
  ...
  save(store, file = paste0("data/intermediary/MCMC_draws_matching/SI/", dependent, "_yearly_", mining_type, "_", matching_model, "_", Sys.time(), ".Rdata"))
  ...
  save(store_impacts, file = paste0("data/cem/sdm_impact_estimates/draws/SI/impacts_", dependent, "_yearly_", mining_type, "_", matching_model, "_", Sys.time(), ".Rdata"))
  ...
  # save as Excel
  write.csv(impact_summary, file = paste0("data/cem/sdm_impact_estimates/summaries/SI/", dependent, "_yearly_", mining_type, "_", window_yrs, "y_window_" , ntot, "draws_", Sys.time(), ".csv"), row.names = FALSE)
