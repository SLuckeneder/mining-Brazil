...
# prepare matrix for relative measure of hectares
X_mining_industrial_rel <- fastDummies::dummy_cols( M %>% dplyr::select(year), select_columns = "year") %>% 
  dplyr::select(-year) *  (M$mining_industrial / M$AREA_mun)
colnames(X_mining_industrial_rel) <- gsub("year", "mining_industrial", colnames(X_mining_industrial_rel))
X_mining_garimpo_rel <- fastDummies::dummy_cols( M %>% dplyr::select(year), select_columns = "year") %>% 
  dplyr::select(-year) *  (M$mining_garimpo / M$AREA_mun)
colnames(X_mining_garimpo_rel) <- gsub("year", "mining_garimpo", colnames(X_mining_garimpo_rel))
...
X <- M %>%
  ...
  as.matrix()
YX <- cbind(Y, X)
write.csv(YX, file = paste0("./data/model_input/SI/YX_hectares_gdp_yearly_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
X <- M %>%
  ...
  dplyr::mutate(mining_industrial_x_pre_2010 = ifelse(year %in% c(2000:2009), mining_industrial, 0),
                ...
                mining_garimpo_x_pre_2010 = ifelse(year %in% c(2000:2009), mining_garimpo, 0),
                mining_garimpo_x_since_2010 = ifelse(year > 2009, mining_garimpo, 0)) %>%
  dplyr::select(-mining_industrial, -mining_garimpo) %>%
  as.matrix()
YX <- cbind(Y, X)
write.csv(YX, file = paste0("./data/model_input/SI/YX_hectares_gdp_pooled_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
X <- M %>% 
  dplyr::select(gdp_capita_growth,
                ...
                precip_norm, elevation,
                year) %>%
  dplyr::bind_cols(X_mining_industrial_rel) %>%
  dplyr::bind_cols(X_mining_garimpo_rel) %>%
  as.matrix()
YX <- cbind(Y, X)
write.csv(YX, file = paste0("./data/model_input/SI/YX_hectares_def_rel_yearly_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
X <- M %>% 
  dplyr::select(gdp_capita_growth,
                ...
                AREA_mun,
                year) %>%
  dplyr::mutate(mining_industrial = mining_industrial / AREA_mun,
                mining_garimpo = mining_garimpo / AREA_mun) %>%
  dplyr::mutate(mining_industrial_x_pre_2010 = ifelse(year %in% c(2000:2009), mining_industrial, 0),
                mining_industrial_x_since_2010 = ifelse(year > 2009, mining_industrial, 0),
                mining_garimpo_x_pre_2010 = ifelse(year %in% c(2000:2009), mining_garimpo, 0),
                mining_garimpo_x_since_2010 = ifelse(year > 2009, mining_garimpo, 0)) %>%
  dplyr::select(-mining_industrial, -mining_garimpo, -AREA_mun) %>%
  as.matrix()
YX <- cbind(Y, X)
write.csv(YX, file = paste0("./data/model_input/SI/YX_hectares_def_rel_pooled_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
X <- M %>% 
  ...
  as.matrix()
YX <- cbind(Y, X)
write.csv(YX, file = paste0("./data/model_input/SI/YX_hectares_def_abs_yearly_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
X <- M %>% 
  ...
  as.matrix()
YX <- cbind(Y, X)
write.csv(YX, file = paste0("./data/model_input/SI/YX_hectares_def_abs_pooled_", from_yr, "-", to_yr, "_", window_yrs, "y", ".csv"), row.names = FALSE)
...
m_dat <- c("YX_hectares_gdp_pooled_2005-2015_5y.csv", "YX_hectares_gdp_yearly_2005-2015_5y.csv",
           "YX_hectares_def_rel_pooled_2005-2015_5y.csv", "YX_hectares_def_rel_yearly_2005-2015_5y.csv",
           "YX_hectares_def_abs_pooled_2005-2015_5y.csv", "YX_hectares_def_abs_yearly_2005-2015_5y.csv")
