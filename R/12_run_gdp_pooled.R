...
# read data
YX <- read.csv2(file = "./data/model_input/YX_gdp_pooled_2005-2015_5y.csv", sep = ",", stringsAsFactors = FALSE) %>% mutate_all(as.numeric)
...
save(store, file = paste0("data/intermediary/MCMC_draws/gdp_pooled_", Sys.time(), ".Rdata"))
...
save(store_impacts, file = paste0("data/impact_estimates/draws/impacts_gdp_pooled_", Sys.time(), ".Rdata"))
...
# save as Excel
write.csv(impact_summary, file = paste0("data/impact_estimates/summaries/gdp_pooled_" , ntot, "draws_", Sys.time(), ".csv"), row.names = FALSE)
